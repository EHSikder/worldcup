export const ar = {
  // Navigation
  nav_home: "الرئيسية",
  nav_predictions: "التوقعات",
  nav_leaderboard: "المتصدرين",
  nav_sign_in: "تسجيل الدخول",
  nav_sign_out: "تسجيل الخروج",
  nav_language: "EN",

  // Homepage - Hero
  hero_badge: "تحدي R-BUILD لكأس العالم",
  hero_title: "توقّع. تنافس. ارتقِ.",
  hero_desc_1: "سجّل توقعاتك، اجمع النقاط، نافس زملاءك، وتصدّر لوحة الترتيب.",
  hero_desc_2: "تنافس مع الجميع وتصدّر القائمة!",
  hero_cta_primary: "توقّع الآن",
  hero_cta_secondary: "لوحة الترتيب",
  hero_left_line1: "توقّع.",
  hero_left_line2: "تنافس.",
  hero_left_line3: "ارتقِ.",
  hero_right_subtitle: "تحدي R-Build لكأس العالم",
  hero_right_title: "توقّع. تنافس.تصدّر.",

  // Homepage - Steps
  steps_title: "كيف تعمل المسابقة؟",
  steps_subtitle: "انضم إلى التحدي في 3 خطوات بسيطة",
  step_1_title: "تسجيل الدخول",
  step_1_desc: "سجّل الدخول وأكمل بياناتك للبدء في المنافسة.",
  step_2_title: "سجّل توقعاتك",
  step_2_desc: "توقع نتائج المباريات والنتائج النهائية قبل انطلاق المباراة.",
  step_3_title: "تصدّر الترتيب",
  step_3_desc: "اجمع النقاط طوال البطولة، تصدّر لوحة الترتيب، وكن بطل R-Build.",

  // Homepage - Scoring
  scoring_title: "نظام النقاط",
  scoring_subtitle: "اكسب نقاطًا مع كل توقع صحيح، واحصل على 10 نقاط إضافية عند توقع النتيجة الدقيقة",
  scoring_winner: "الفائز الصحيح",
  scoring_exact: "النتيجة الدقيقة",
  scoring_pts: "نقطة / مباراة",
  scoring_exact_bonus_title: "⚡ مكافأة النتيجة الدقيقة",
  scoring_exact_bonus_desc: "اكسب نقاطاً إضافية في أي جولة إذا توقعت النتيجة الدقيقة تماماً!",

  // Scoring rounds
  scoring_round_group: "دور المجموعات",
  scoring_round_r32: "دور الـ32",
  scoring_round_r16: "دور الـ16",
  scoring_round_qf: "ربع النهائي",
  scoring_round_sf: "نصف النهائي",
  scoring_round_3rd: "المركز الثالث",
  scoring_round_final: "بطل البطولة",

  // Homepage - Teams
  teams_title: "المنتخبات الـ48",
  teams_subtitle: "12 مجموعة، 48 منتخبًا، ومتصدر واحد فقط.",
  group: "مجموعة",

  // Homepage - Bottom CTA
  cta_title: "هل أنت مستعد للتحدي؟",
  cta_subtitle: "انضم إلى التحدي، وتفوق على المنافسين، وأثبت أنك الأفضل في توقع نتائج البطولة!",
  cta_btn: "ابدأ التوقع الآن",

  // Footer
  footer_brand_name: "R BUILD Predictor",
  footer_brand_desc: "توقع نتائج البطولة، تنافس مع الأصدقاء والزملاء، وتصدر لوحة المتصدرين في تحدي توقعات R BUILD.",
  footer_nav_title: "التنقل",
  footer_nav_home: "الرئيسية",
  footer_nav_predictions: "التوقعات",
  footer_nav_leaderboard: "المتصدرين",
  footer_nav_login: "تسجيل الدخول",
  footer_legal_title: "قانوني",
  footer_terms: "شروط الخدمة",
  footer_privacy: "سياسة الخصوصية",
  footer_copy: "جميع الحقوق محفوظة.",
  footer_powered: "بدعم من R BUILD والشركاء",
  footer_text: "© 2026 تحدي توقعات R BUILD. جميع الحقوق محفوظة.",

  // Leaderboard
  lb_title: "لوحة المتصدرين",
  lb_subtitle: "أفضل المتوقعين مصنفين حسب إجمالي النقاط المكتسبة",
  lb_search: "البحث عن لاعبين...",
  lb_empty: "لم يتم تقديم أي توقعات بعد. كن أول من يملأ توقعاتك.",
  lb_prize: "المتصدر",
  lb_points: "نقاط",
  lb_col_rank: "المركز",
  lb_col_user: "اسم المستخدم",
  lb_col_team: "الفريق",
  lb_grand_prize_label: "المتصدر الأول",
  lb_grand_prize_tagline: "توقّع وتنافس",
  lb_grand_champion_title: "تصدّر الترتيب",
  lb_grand_champion_subtitle: "المتوقع الأعلى نقاطاً يحوز اللقب",

  // Predictions
  pred_title: "توقعات المباريات",
  pred_subtitle: "توقع الفائز والنتيجة الدقيقة لكل مباراة.",
  pred_unsaved: "تغييرات غير محفوظة",
  pred_submit: "إرسال التوقعات",
  pred_success: "تم حفظ التوقعات بنجاح!",
  pred_match: "مباراة",
  pred_exact_score: "توقع النتيجة الدقيقة (مكافأة +10 نقاط)",
  pred_draw: "تعادل",
  pred_prize_label: "تنافس وارتقِ",
  pred_prize_tagline: "تصدّر الترتيب",
  pred_rank_label: "رتبة",
  pred_leader_label: "المتصدر",
  pred_you: "(أنت)",

  // Login
  login_title: "مرحباً بعودتك",
  login_subtitle: "سجل الدخول باستخدام Google للبدء في التوقع.",
  login_google: "تسجيل الدخول باستخدام Google",

  // Complete Profile
  profile_title: "أكمل ملفك الشخصي",
  profile_subtitle: "اختر فريقك المفضل لإنهاء إعداد حسابك.",
  profile_name: "الاسم الكامل",
  profile_email: "عنوان البريد الإلكتروني",
  profile_team: "الفريق المفضل",
  profile_team_ph: "ابحث عن فريق...",
  profile_submit: "إنهاء الإعداد",
  profile_saving: "جاري الحفظ...",
};
