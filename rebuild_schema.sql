-- ============================================================
-- FIFA WORLD CUP 2026 KNOCKOUT PREDICTOR — REBUILD SCHEMA
-- Run this entire file in your Supabase SQL Editor to wipe and rebuild!
-- ============================================================

-- 1. WIPE EXISTING SCHEMA
DROP VIEW IF EXISTS bracket_view CASCADE;
...
DROP TABLE IF EXISTS bracket_locks CASCADE;
...
DROP TYPE IF EXISTS lock_reason CASCADE;
...
CREATE TABLE teams (
  ...
  flag_url TEXT,
  flag_code CHAR(6),
  worldcupapi_id INTEGER, -- Changed from api_football_id
...
CREATE INDEX idx_teams_worldcupapi_id ON teams(worldcupapi_id);
...
CREATE TABLE matches (
  ...
  venue TEXT,
  city TEXT,
  worldcupapi_fixture_id INTEGER, -- Changed from api_football_fixture_id
...
CREATE INDEX idx_matches_worldcupapi_fixture ON matches(worldcupapi_fixture_id);
...
-- 4. PREDICTIONS — User bracket predictions
...
-- 8. SYNC_LOG — Track WorldCupAPI sync jobs
...
CREATE TRIGGER update_bracket_locks_updated_at BEFORE UPDATE ON bracket_locks FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "users_read_own" ON users FOR SELECT TO authenticated USING (id = (SELECT auth.uid()));
...
CREATE POLICY "predictions_read_own" ON predictions FOR SELECT TO authenticated USING (user_id = (SELECT auth.uid()));
CREATE POLICY "predictions_insert_own" ON predictions FOR INSERT TO authenticated WITH CHECK (user_id = (SELECT auth.uid()));
CREATE POLICY "predictions_update_own" ON predictions FOR UPDATE TO authenticated USING (user_id = (SELECT auth.uid()) AND is_locked = false) WITH CHECK (user_id = (SELECT auth.uid()));
...
(103, 'third_place', 'Loser Match 101', 'Loser Match 102', '2026-07-18 21:00:00+00', 'Hard Rock Stadium', 'Miami', NULL, NULL),
(104, 'final', 'Winner Match 101', 'Winner Match 102', '2026-07-19 21:00:00+00', 'MetLife Stadium', 'New York/New Jersey', NULL, NULL);

-- ============================================================
-- SEED DATA: SCORING RULES & LOCKS
-- ============================================================
...
CREATE POLICY "champion_pred_read_own" ON champion_predictions FOR SELECT TO authenticated USING (user_id = (SELECT auth.uid()));
CREATE POLICY "champion_pred_insert_own" ON champion_predictions FOR INSERT TO authenticated WITH CHECK (user_id = (SELECT auth.uid()));
CREATE POLICY "champion_pred_update_own" ON champion_predictions FOR UPDATE TO authenticated USING (user_id = (SELECT auth.uid())) WITH CHECK (user_id = (SELECT auth.uid()));
CREATE TRIGGER update_champion_predictions_updated_at BEFORE UPDATE ON champion_predictions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
